...
class DuGet(object):
    def __init__(self, task_q=None, results_q=None, sleep=4):
    ...
    @staticmethod
    def calc_hash(value, method='md5'):
    ...
    def get_pic(self, url, articleNumber, price, count):
        try:
            res = requests.get(url, stream=True)
            md5 = self.calc_hash(res.content)
            with open(os.path.join('pic', "{}({}_{})_{}.jpg".format(articleNumber, price, count, md5)), 'wb') as f:
                f.write(res.content)
        except Exception as e:
            logger.error("get_pic {} {}".format(url, e))

    def search_keywords(self, keywords, sortMode=1, sortType=1):
        # 关键词搜索商品接口
        others = []
        for page in range(20):
            page_size = 20
            sign = self.sign('limit{}page{}showHot-1sortMode{}sortType{}title{}unionId19bc545a393a25177083d4a748807cc0'.format(page_size, page, sortMode, sortType, keywords))
...
class MailNotify(object):
    def __init__(self, _mail_host, _mail_pass, _sender, _receivers, _sleep=30):
...
def merge_pic(per_count=9):
    ...
    for fn in file_list:
        if fn.endswith('.jpg'):
            ...
            articleNumber = fn[:rfind_index]
            im = Image.open(os.path.join(path_base, fn)).convert('RGBA')
            # 新建一个空白图片,尺寸与打开图片一样
    ...
    for i in im_list:
        done_index += 1
        new_img = i.resize((500, 320), Image.BILINEAR)
        ims.append(new_img)
        if len(ims) >= per_count:
            # 单幅图像尺寸
            ...
            # 保存图片
            result_index = done_index/per_count
            tmp_name = os.path.join(path_base, 'results.{}.jpg'.format(result_index))
            results_list.append(tmp_name)
            result.save(tmp_name, 'png')
            ims = []
