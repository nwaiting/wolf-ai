...
class CollectData():
    ...
    """
    def __init__(self, keyword, startTime, interval='30', flag=True, begin_url_per = "http://s.weibo.com/weibo/"):
            ...
            for tryNum in range(maxTryNum): ##0-3
                try:
                    ...
                    if tryNum < (maxTryNum-1):
                        ...
                        print('Internet Connect Error! {0}'.format(e))
                        ...
                        if n > 0:
                            ...
                            ## 有结果的页面
                                ...
                                for p1 in ps1:
                                    ...
                                    addr = 'http:' + addr
                                    print('addr ', addr)
                                    html2 = urllib.request.urlopen(addr, timeout=12)
                                    #print('html2.info() ', html2.info())
                                    #print('dir ', dir(html2))
                                    #print(html2.headers)
                                    data2 = html2.read().decode('utf-8', 'ignore')
                                    ...
                                    #print('str(txt) ', type(str(txt.decode('utf-8'))))
                                    print('txt ', dir(txt), type(txt))
                                    print('txt ', txt[0].strip())
                                    print('str(txt) ', str(txt))
                                    if name and str(txt) and name not in name_filter:
                                        name_filter.add(name)
                                        with open('aaaaaaaaaaaaa.txt', 'wb') as fd:
                                            contents = "{0},{1},{2}\n".format(name,addr1,txt)
                                            fd.write(contents.encode('utf-8'))
                                        """
                                        oldWb = xlrd.open_workbook(r'weiboData.xls', formatting_info=True)
...
def main():
    ...
    while True:
        ...
        Ndays = int(input('Enter the recent N days messages:'))
